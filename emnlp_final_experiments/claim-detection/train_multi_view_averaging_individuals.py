...
from datareader import MultiDomainTwitterDataset
...
def train(
        ...
        scheduler: LambdaLR,
        validation_evaluators: List[MultiDatasetClassificationEvaluator],
        n_epochs: int,
        device: AnyStr,
        log_interval: int = 1,
        patience: int = 10,
    ...
    while epoch_counter < n_epochs:
        ...
        for v,validation_evaluator in enumerate(validation_evaluators):
            ...
            if F1 > best_f1s[v]:
                ...
                if v < len(train_dls):
                    torch.save(model.module.domain_experts[v].state_dict(), f'{model_dir}/{Path(wandb.run.dir).name}/model_{domain_name}_{v}.pth')
                else:
                    torch.save(model.module.shared_bert.state_dict(), f'{model_dir}/{Path(wandb.run.dir).name}/model_{domain_name}_{v}.pth')
                patience_counter = 0
                # Log to wandb
                wandb.log({
...
def attention_grid_search(
    ...
    param_dict = {1:list(range(0,11)), 2:list(range(0,11)),3:list(range(0,11)),4:list(range(0,11)), 5:list(range(0,11))}
...
if __name__ == "__main__":
    ...
    wandb.init(
        project="domain-adaptation-twitter-emnlp",
        name=args.run_name,
        config={
            "epochs": n_epochs,
            "learning_rate": lr,
    ...
    # Create the dataset
    all_dsets = [MultiDomainTwitterDataset(
        args.dataset_loc,
        [domain],
    ...
    for i in range(len(all_dsets)):
        ...
        if args.pretrained_bert is not None:
            init_weights = {k: v for k, v in torch.load(args.pretrained_bert).items() if "classifier" not in k}
            for m in model.module.domain_experts:
                model_dict = m.state_dict()
                model_dict.update(init_weights)
                m.load_state_dict(model_dict)
            model_dict = model.module.shared_bert.state_dict()
            model_dict.update(init_weights)
            model.module.shared_bert.load_state_dict(model_dict)
        ...
        for v in range(len(train_dls)):
            model.module.domain_experts[v].load_state_dict(torch.load(f'{args.model_dir}/{Path(wandb.run.dir).name}/model_{domain}_{v}.pth'))
        model.module.shared_bert.load_state_dict(torch.load(f'{args.model_dir}/{Path(wandb.run.dir).name}/model_{domain}_{len(train_dls)}.pth'))
