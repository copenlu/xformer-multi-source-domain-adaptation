...
from metrics import MultiDatasetClassificationEvaluator, DomainClassifierEvaluator
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained_model", help="Directory with weights to initialize the model with", type=str, default=None)
        ...
        if args.pretrained_model is not None:
            for v in range(len(train_dls)):
                model.module.domain_experts[v].load_state_dict(
                    torch.load(f'{args.pretrained_model}/model_{domain}_{v}.pth'))
        # (val_loss, acc, P, R, F1), _ = validation_evaluator.evaluate(model)
        # print(f"Validation acc starting: {acc}")
        else:

            # Create the model
            model = torch.nn.DataParallel(MultiViewTransformerNetworkDomainClassifierIndividuals(
                bert_model,
                bert_config,
                shared_bert,
                len(train_dls)
