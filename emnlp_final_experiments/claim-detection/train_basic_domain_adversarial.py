...
if __name__ == "__main__":
    ...
    parser.add_argument("--supervision_layer", help="The layer at which to use domain adversarial supervision", default=12, type=int)
    ...
    if args.full_bert:
        ...
        bert_config = BertConfig.from_pretrained(bert_model, num_labels=2, output_hidden_states=True)
    ...
    for i in range(len(all_dsets)):
        ...
        model = torch.nn.DataParallel(DomainAdversarialBert(
            bert,
            n_domains=len(train_dls), supervision_layer=args.supervision_layer
        )).to(device)
