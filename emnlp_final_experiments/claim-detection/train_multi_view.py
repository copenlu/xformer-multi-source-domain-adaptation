...
if __name__ == "__main__":
    ...
    parser.add_argument("--ensemble_avg_learned", help="Use learned averaging for the ensembling method", action="store_true")
    ...
    for i in range(len(all_dsets)+1):
        ...
        if args.ensemble_basic:
            model_class = MultiViewTransformerNetworkAveraging
        elif args.ensemble_avg_learned:
            model_class = MultiViewTransformerNetworkLearnedAveraging
        else:
            model_class = MultiViewTransformerNetworkProbabilities

        model = torch.nn.DataParallel(model_class(
            multi_xformer,
            shared_bert
        )).to(device)
        ...
        if args.ensemble_avg_learned:
            weights = model.module.alpha_params.cpu().detach().numpy()
